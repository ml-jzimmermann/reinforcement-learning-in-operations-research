...
for seed in seeds:
    hyperparameters = {
        ...
        'plot_progress': False,
        'ylim': (-200, 200),
        'tag': f'warehouse_v7_thesis_big_less_centered_dqn_{seed}'
    }
    ...
    warehouse = WarehouseV7(num_aisles=hyperparameters['num_aisles'], rack_height=hyperparameters['rack_height'],
                            ...
                            seed=hyperparameters['np_seed'], center_target=True)

    agent = QAgent(env=warehouse, model=DQN, config=hyperparameters)
    agent.compile()
    agent.fit()
